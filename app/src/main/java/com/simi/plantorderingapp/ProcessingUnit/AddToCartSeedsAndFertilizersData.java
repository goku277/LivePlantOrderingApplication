...
public class AddToCartSeedsAndFertilizersData {
  /*  public static void main(String[] args) {
    ...
    public Set<Set<String>> init(String data, String username, String usermobile) {
        ...
        for (String s : split) {
            String name = "", price = "", imageUri = "", quantity= "", seedsqty= "";
            set1 = new LinkedHashSet<>();
            if (s.contains(username) && s.contains(usermobile)) {
                if (s.contains("Name:") && s.contains("Details:")) {
                    name = s.substring(s.indexOf("Name:"), s.indexOf("Details:")).replace("Name:", "Name:\t");
                    set1.add(name.trim());
                }
                ...
                if (s.contains("imageUri:") && s.contains("Quantity:")) {
                    imageUri = s.substring(s.indexOf("imageUri:"), s.indexOf("Quantity:")).trim();
                    set1.add(imageUri);
                }
                if (s.contains("Quantity:") && s.contains("seedsqty:")) {
                    quantity= s.substring(s.indexOf("Quantity:"), s.indexOf("seedsqty:"));
                    set1.add(quantity);
                }
                if (s.contains("seedsqty:")) {
                    seedsqty= s.substring(s.indexOf("seedsqty:"));
                    set1.add(seedsqty);
                }
                detailsSet.add(set1);
            }
